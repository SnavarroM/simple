        ...
        <table class="table mt-3">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="tarea-estado" data-tramite="{{ $t['id'] }}">
                        {{ $t['estado'] ? 'En curso' : 'Completado' }}
                    </td>
                    <td class="tarea-etapa-actual" data-tramite="{{ $t['id'] }}">
                    ...
                    <td style="text-align: right;">
                        ...
                        @if(in_array('super', explode(',', Auth::user()->rol)))
                            <a class="btn btn-danger eliminar-tarea" href="#" data-tramite="{{ $t['id'] }}">
...
@section('script')
    <script type="text/javascript">
        ...
        function eliminarTramite(tramiteId, estado, etapa) {
            var url = "/backend/seguimiento/ajax_auditar_eliminar_tramite/" + tramiteId + "?estado="+estado+"&etapa="+etapa;
            $("#modal").load(encodeURI(url));
            $("#modal").modal();
            return false;
        }
        ...
        $(document).ready(function() {
            $(".eliminar-tarea").on("click", function() {
                let tramiteId = $(this).data("tramite");
                let estado, etapa;

                $(".tarea-estado").each(function(index, element) {
                    if ($(element).data("tramite") == tramiteId) {
                        estado = $(element).text()
                        estado = estado.split("\n").join("").trim();
                    }
                });

                $(".tarea-etapa-actual").each(function(index, element) {
                    if ($(element).data("tramite") == tramiteId) {
                        etapa = $(element).text()
                        etapa = etapa.split("\n").join("").trim();
                    }
                ...
                eliminarTramite(tramiteId, estado, etapa)
            });
