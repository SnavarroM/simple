...
use App\Helpers\GoogleAnalyticsHelper;
...
class StagesController extends Controller
{
    ...
    {   
        ...
        if (!$paso) {
            ...
            //TODO Refectorizar con GoogleAnalyticsHelper
    ...
    public function googleAnalytics( $extra, $etapa, $paso, $secuencia ) {
        
        $events = array();

        if(!isset($extra['mostrar_hit'])) {                        
            $events = array_merge(
                        $events, 
                        GoogleAnalyticsHelper::getGAEventsArray($etapa->id, $etapa->Tarea->id, GoogleAnalyticsHelper::$INSTANTE_ANTES),
                        GoogleAnalyticsHelper::getStepGAEventsArray($etapa->id, $paso->id, GoogleAnalyticsHelper::$INSTANTE_ANTES)
                    );
        } else{            
            // Eventos GA despues del paso anterior(si existen). Se agrega aqui ya que en el metodo ejecutar_form no hay un return
            Log::info('############ PREV PASOOOOOOOO');
            $prev_paso = $etapa->getPasoEjecutable($secuencia - 1);
            if($prev_paso)
            {
                // Eventos GA despues del paso anterior(si existen)
                $gaEvents = GoogleAnalyticsHelper::getStepGAEventsArray($etapa->id, $prev_paso->id, GoogleAnalyticsHelper::$INSTANTE_DESPUES);
                $events = array_merge( $events, $gaEvents );                
            }
            
            $events = array_merge( 
                    $events, 
                    GoogleAnalyticsHelper::getStepGAEventsArray($etapa->id, $paso->id, GoogleAnalyticsHelper::$INSTANTE_DESPUES),
                    GoogleAnalyticsHelper::getGAEventsArray($etapa->id, $etapa->Tarea->id, GoogleAnalyticsHelper::$INSTANTE_DESPUES)
                );
        }
        return $events;
    }
    ...
    public function ejecutar_form(Request $request, $etapa_id, $secuencia)
    {
        ...
        if ($modo == 'edicion') {
                ...
                if ($c->isEditableWithCurrentPOST($request, $etapa_id)) {
                    ...
                    if($c->tipo=='comunas'){
                        ...
                        if(!is_null($dato->valor)){
                            ...
                            $file = Doctrine::getTable('File')->findOneByFilenameAndCampoIdAndTramiteId($dato->valor, $c->id, $etapa->Tramite->id);
                            if($file) {
                                $fileTransferExtra = (array) $file->getExtra();
                                $fileTransferExtra['file_confirmed'] = true;
                                $file->extra = $fileTransferExtra;

                                $file->save();

                                File::eliminaArchivoSinConfirmarOAntiguos([
                                    "tipo" => "dato",
                                    "campoId" => $c->id,
                                    "tramiteId" => $etapa->Tramite->id,
                                    "clearConfirmed" => true,
                                    "exceptionFileId" => $file->id
                                ]);
                            }
                        ...
                        if(!is_null($dato->valor) && isset($dato->valor->filename)){
                            ...
                            if($file) {
                                ...
                                // $file_confirmed=false es utilizada para identificar los registros a eliminar de la tabla file
                                $fileTransferExtra = (array) $file->getExtra();
                                $fileTransferExtra["file_confirmed"] = true;
                                $file->extra = $fileTransferExtra;
            ...
            if (!$prox_paso) {
                ...
                if( $prox_paso->getReadonly() ) {

                    // EVENTOS GA si solo hay una tarea                    
                    $eventos = array();
                    $busca_evento_analytics_antes = GoogleAnalyticsHelper::getStepAnalyticsEvents( 
                        [ GoogleAnalyticsHelper::$INSTANTE_ANTES, GoogleAnalyticsHelper::$INSTANTE_DESPUES ],    
                        $etapa_id, 
                        $prox_paso->id );
                    $busca_evento_analytics_despues = GoogleAnalyticsHelper::getTaskAnalyticsEvents( 
                            GoogleAnalyticsHelper::$INSTANTE_DESPUES, 
                            $etapa_id, 
                            $etapa->Tarea->id );
                    
                    \Log::debug('eventos ga despues del paso ' . $prox_paso->id );
                    Log::info("###Lo que trae busca_evento_analytics_antes : " . $busca_evento_analytics_antes);
                    Log::info("###Lo que trae busca_evento_analytics_despues : " . $busca_evento_analytics_despues);

                    $eventos = array_merge( $eventos,
                        GoogleAnalyticsHelper::createGAArrayEvents($busca_evento_analytics_antes),
                        GoogleAnalyticsHelper::createGAArrayEvents($busca_evento_analytics_despues) );
                    $extra_etapa['mostrar_hit'] = false;
                    if (count($eventos) > 0) {                        
                        $data['extra']['eventos'] = $eventos;
                        $data['extra']['analytics'] = true;
        ...
        foreach ($tramites as $t) {
            ...
            if (count($files) > 0) {
                ...
                foreach ($files as $f) {
                    ...
                    if($ruta_base != 's3'){
                        $tarea = $tarea->where( DB::raw('TRIM( BOTH \'"\' from dato_seguimiento.valor)'), $f->filename);
                    }
