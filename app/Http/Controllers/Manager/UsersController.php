...
class UsersController extends Controller
{
    ...
    {   
        $user_search = $request->input('user_search', null);

        $usuarios = UsuarioBackend::orderBy('id', 'desc');

        if (null != $user_search) {
            $usuarios = $usuarios
                ->where('email', 'like', "%$user_search%")
                ->orWhere('nombre', 'like', "%$user_search%")
                ->orWhere('apellidos', 'like', "%$user_search%");
        }

        $usuarios = $usuarios->orderBy('id')->paginate(20);

        $data['content'] = view('manager.users.index', [
            'title' => 'Usuarios Backend',
            'usuarios' => $usuarios,
