...
class dateHelper{
    ...
    function getFeriados($anio=null, $mes=null){
        $feriados = array();
        $res = json_decode(file_get_contents(env('API_FERIADOS').$anio."/".$mes.".json"));
        if (is_array($res)) {
            foreach ($res as $r) {
                $feriados[] = $r->fecha;
            }
        }
        Log::debug('============ [ Resultado de .json Feriados] ============', [
            "respuesta_feriados_s3" => $res,
            "feriados_procesados" => $feriados
        ]);
