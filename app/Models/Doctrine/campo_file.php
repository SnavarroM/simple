...
class CampoFile extends Campo
{
    ...
    protected function display($modo, $dato, $etapa_id = false)
    {
        ...
        //eliminando files no asociados al tramite al refrescar formulario
    ...
    public function backendExtraValidate(Request $request)
    {
        ...
        if (isset($extraMetadata)) {
            $request->validate(['extra' => [
                function ($attribute, $extra, $fail) {
                    if (!array_key_exists('maxfilesize', $extra) || null == $extra['maxfilesize']) {
                        $fail('Debe definir un tamaño maximo para el archivo.');
                    } else {
                        // existe y no es null
                        $sizeOriginal = $extra['maxfilesize'];
                        $maxSize = $extra['maxfilesize'];
                        if ( !is_numeric($maxSize) ) {
                            $fail("El tamaño máximo definido $sizeOriginal no es válido.");
                        } else if (0 >= $maxSize) {
                            $fail('Debe definir un tamaño maximo con valor entero positivo.');
                        }
                    }

                    if (!array_key_exists('filetypes', $extra)) {
                        $fail('Debe seleccionar al menos un tipo de extensión para su archivo.');
                    }
                }
            ]]);
        }
