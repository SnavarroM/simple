        ...
        if (isset($extraMetadata)) {
            $request->validate(['extra' => [
                function ($attribute, $extra, $fail) {
                    if (!array_key_exists('max_size', $extra) || null == $extra['max_size']) {
                        $fail('Debe definir un tamaño maximo para el archivo.');
                    } else {
                        // existe y no es null
                        $sizeOriginal = $extra['max_size'];
                        $maxSize = $extra['max_size'];
                        if ( !is_numeric($maxSize) ) {
                            $fail("El tamaño máximo definido $sizeOriginal no es válido.");
                        } else if (0 >= $maxSize) {
                            $fail('Debe definir un tamaño maximo con valor numérico positivo.');
                        }
                    }
