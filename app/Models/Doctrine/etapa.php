...
class Etapa extends Doctrine_Record
{
    ...
    public function asignar($usuario_id)
    {
        ...
        if ($this->Tarea->acceso_modo == 'grupos_usuarios' || $this->Tarea->acceso_modo == 'registrados') {
            $usuario = User::where('usuario',$usuario_id)->whereNotNull('cuenta_id')->first();
            if (!$usuario) {
                $usuario = Doctrine::getTable('Usuario')->find($usuario_id);
            }
            $usuario_id = !is_null($usuario->cuenta_id) ? $usuario->id : NULL;
            Log::info("3 Acceso usuario : " . $usuario_id);
        }
        ...
        if ($this->Tarea->asignacion_notificar && !is_null($usuario_id)) {
            $usuario = Doctrine::getTable('Usuario')->find($usuario_id);
            if ($usuario->email) {
    ...
    public function notificarTareaPendienteOld($asunto,$mensaje)
    {
        if ($this->Tarea->asignacion_notificar) {
